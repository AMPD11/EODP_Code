...
def plot_band(band):
    ...
    plt.plot(x, y_ref,  label="Ref TOA",  linewidth=1.5)
    plt.plot(x, y_test, label="Test TOA", linewidth=1.2)
    ...
    out_png = os.path.join(OUT_DIR, f"isrf_compare_{band}.png")
    plt.tight_layout()
    plt.savefig(out_png)
    plt.close()
