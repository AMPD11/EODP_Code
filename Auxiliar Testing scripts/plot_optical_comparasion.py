...
FNAME = "ism_toa_optical_{band}.nc"  # <-- optical
...
def plot_band(band):
    ...
    plt.plot(x, y_ref,  label="Ref TOA (optical)", linewidth=1.5)
    plt.plot(x, y_test, label="Test TOA (optical)", linewidth=1.2)
    plt.title(f"TOA optical comparison — {band}")
    plt.xlabel("ACT [-]")
    plt.ylabel("Radiance [mW/m²/sr]")
    plt.grid(True, alpha=0.3)
    plt.legend()

    out_png = os.path.join(OUT_DIR, f"optical_compare_{band}.png")
    plt.tight_layout()
    plt.savefig(out_png)
    plt.close()
